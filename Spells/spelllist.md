## Spell List Lookup
```python
!servalias spelllist embed
{{c,lv,g="&1&".lower(),0 if "cantrip" in "&2&" else int("&2&"),load_json(get_gvar("195df130-624a-4ed0-a4c6-21580902a4f7"))}}
-title "Spell Lookup for: \"{{c}}\" at level \"{{lv}}\""
-desc "{{"\n".join([x for x in g[lv] if c in x])}}"
```
...
[
  ...
  [
    ...
    "Hold Person (bard, druid, sorcerer, warlock)",
    "Gentle Repose (cleric)",
    "Gust of Wind (druid, sorcerer)",
    ...
    "Protection of Poison (cleric, druid, poison, ranger)",
  ...
  [
    ...
    "Remove Curse (cleric, paladin, warlock)",
